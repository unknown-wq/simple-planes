...
120 exactly. The next format bump will need `max_format` raised with it. (It came in pre-3; see
below.)
...
## 26.3-pre-2 → 26.3-pre-3

The third pre-release moved the module to `26.3-pre-3` (world version 5019, resource pack 97 /
data pack 121) and to Fabric API **0.160.2+26.3**. The loader is unchanged at 0.19.5.

**One resource change, no source change.** The data pack format bump the pre-2 note predicted
arrived: 120 became 121, one past this pack's `max_format`, so `pack.mcmeta` now declares 88-121.
Everything else compiles and builds untouched, and all four access widener entries still resolve.

The two API changes pre-3 does carry are both absent from this mod. `Util.OS` lost its URI and file
