...
public final class GunshipSortie {
    ...
    /**
     * {@code LivingEntity#hurtServer} sets {@code damageCooldownTime = 20} on a hit and refuses any
    ...
    /** The value {@code damageCooldownTime} must be at or below when a round arrives, for it to count. */
     ...
     * {@code damageCooldownTime = 20} on every hit and, while that is above 10, ignores any subsequent
     ...
     *
     * <p>26.3 renamed this timer: {@code Entity#invulnerableTime} is private now, and the
     * LivingEntity damage cooldown the rule is written about is {@code LivingEntity#damageCooldownTime},
     * which is public and carries the identical 20-tick / >10 semantics.
     *
     * <p>So the gunship counts the flight time. A round is fired only if it will arrive after the
     * window closes — both the window the target is in now ({@code damageCooldownTime}) and the one
    ...
    private boolean worthShooting(LivingEntity mob, double flightTicks) {
        ...
        if (mob.damageCooldownTime - (arrival - ticks) > IMMUNITY_THRESHOLD) {
            return false;
        }
